...
public class RawDataListener {
    ...
    public void listen(ConsumerRecord<?, ?> record, Acknowledgment ack) throws IOException {
        ...
        try {
            BuyResultEnum resultEnum = secKillService.handle(request.getUserId(), request.getProductId(), request.getBuyNumber(), false);
            //同步发送秒杀结果消息
            kafkaTemplate.send(responseTopic, new String(serializer.serialize(resultEnum))).get();
            //请求消费手动提交
            ack.acknowledge();
        } catch (Exception s) {
